...
import { dailyCoachLessons, verifyDailyCoachAnswer } from '$lib/content/daily-coach';
...
export const actions: Actions = {
	complete: async ({ request, locals }) => {
		...
		const verification = verifyDailyCoachAnswer(form.get('day'), form.get('answer'));
		if ('error' in verification) return fail(400, { error: verification.error });
		const { day, lesson } = verification;
